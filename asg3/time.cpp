...
void Clock::display() const {
  ...
  IOManager::getInstance()
    .printMessageValueAt("frames in sec: ", lastFrames, 10, 20);
}
...
void Clock::getFps() {
    int pointer=n-1;
    while(pointer>0){
        timeStamp[pointer]=timeStamp[pointer-1];
        pointer--;
    }
    timeStamp[0]=SDL_GetTicks();
    float deltaT=(float)(timeStamp[0]-timeStamp[n-1])/(n-1)/1000;
    float fps=1.0/deltaT;
  IOManager::getInstance()
    .printMessageValueAt("fps: ", fps, 10, 10);
}
